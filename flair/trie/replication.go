...
func (r *Replicator) SequentialAck(key string, f ReplicatedAckFunc) error {
	...
	for i := 0; i < r.Replicas; i++ {
		...
		if i < r.Replicas-1 {
			log.Error("Error reading from replica for %s: %v. Will retry on next replica.", key, err)
		} else {
			log.Error("Error reading from replica for %s: %v.", key, err)
		}
		offset += r.increment
	}
	if success {
...
func (r *Replicator) Parallel(key string, f ReplicatedFunc) error {
	...
	if err := g.Wait(); err != nil {
		if len(err.Errors) < r.Replicas {
			log.Error("Writes to some replicas for %s failed: %v", key, err)
			return nil
		}
