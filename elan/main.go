...
var opts = struct {
	...
	SecondaryStorage string        `long:"secondary_storage" description:"URL defining secondary storage endpoint to use which will receive lookups if the primary doesn't have them."`
...
func main() {
	...
	rpc.ServeForever(opts.GRPC, opts.Storage, opts.SecondaryStorage, opts.FileCache.Path, int64(opts.FileCache.MaxSize), opts.Parallelism)
}
