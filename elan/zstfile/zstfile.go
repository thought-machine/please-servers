// Package zstfile provides a blob implementation akin to fileblob,
// but with (optional) zstd compression to save space.
//
...
const xattrName = "user.elan_zst"

// URLOpener opens file bucket URLs like "zstfile:///foo/bar/baz".
...
func (b *bucket) NewRangeReader(ctx context.Context, key string, offset, length int64, opts *driver.ReaderOptions) (driver.Reader, error) {
	...
	if offset > 0 {
		if _, err := zstr.Seek(offset, io.SeekStart); err != nil {
			return nil, err
		}
	}
	r := io.Reader(zstr)
...
// Copy is not implemented for zstfile.
...
// SignedURL is not implemented for zstfile.
